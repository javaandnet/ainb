export default class Recorder {
    ...
    analyser;
    size = 0;                       // 录音文件总长度
    lBuffer = [];      // pcm音频数据搜集器(左声道)
    rBuffer = [];      // pcm音频数据搜集器(右声道)
    PCM;                               // 最终的PCM数据缓存，避免多次encode
    tempPCM = [];          // 边录边转时临时存放pcm的
    audioInput;
    inputSampleRate = 0;              // 输入采样率
    inputSampleBits = 16;         // 输入采样位数
    outputSampleRate;             // 输出采样率
    oututSampleBits;              // 输出采样位数
    source;                            // 音频输入
    recorder;
    stream;                            // 流
    littleEdian = false;                 // 是否是小端字节序
    fileSize = 0;                 // 录音大小，byte为单位
    duration = 0;                 // 录音时长
    needRecord = true;             // 由于safari问题，导致使用该方案代替disconnect/connect方案
    // 正在录音时间，参数是已经录了多少时间了

    // onprocess 替代函数，保持原来的 onprocess 向下兼容
    onprogress = null;
    // onplay: () => {};                  // 音频播放回调
    // onpauseplay: () => void;             // 音频暂停回调
    // onresumeplay: () => void;            // 音频恢复播放回调
    // onstopplay: () => void;              // 音频停止播放回调
    // onplayend: () => void;               // 音频正常播放结束
    ...
    async destroyRecord() {
        ...
        return await this.closeAudioContext();
    }
